...
Queue.prototype.push = function (task, trigger, callback) {
  if (!callback && typeof trigger === 'function') {
    callback = trigger;
    trigger = false;
  }
  ...
  if (this.items.length >= this.maxSize) {
    const error = new Error(`Queue is full (${this.maxSize}). Cannot create run for task ${task.id}. Consider investigating...`);
...
Queue.prototype.getNext = function (runningTaskIds) {
  ...
  for (let index = 0; index < self.items.length; index++) {
    if (runningTaskIds.indexOf(self.items[index].task.id) === -1) {
      const run = self.items.splice(index, 1)[0];
