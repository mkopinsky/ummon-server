...
const DependencyGraph = require('dependency-foo');
...
const TASK_FIELDS = ['name', 'collection', 'command', 'cwd', 'env', 'trigger', 'enabled', 'description'];
...
function Ummon(options) {
  ...
  d.run(function () {
    ...
    if (self.config.tasksPath) {
      const oldAutoSave = self.config.autoSave;
...
function loadConfig(options) {
  // Load config
  const defaults = require(path.join(__dirname, '../config/config.defaults.json')); // Load config.defaults.json
  let config = {};
  if (!options) options = {};
...
Ummon.prototype.setupListeners = function (taskid) {
  ...
  self.on('task.*', function (taskid) {
    const wildCardTriggerTasks = _.filter(self.tasks, function (task) {
  ...
  self.on('worker.complete', function (completedRun) {
    ...
    if (completedRun.task.id) {
      const task = self.tasks[completedRun.task.id];
      const status = (completedRun.exitCode === 0) ? 'success' : 'error';
      ...
      const refs = self.getTaskReferences(completedRun.task.id, status);
...
Ummon.prototype.getTaskIds = function (selector) {
  ...
  const taskids = Object.keys(self.tasks);
...
Ummon.prototype.createTask = function (config, callback) {
  ...
  const id = `${config.collection}.${config.name}`;
...
Ummon.prototype.updateCollectionAndTasks = function (config, callback) {
  ...
  const newTasks = Object.keys(config.tasks);
  const oldTasks = _.pluck(_.where(self.tasks, { collection: config.collection }), 'name');
  const allTasks = _.union(oldTasks, newTasks);
...
Ummon.prototype.getTask = function (tid, callback) {
  ...
  const task = self.getTaskSync(tid);
...
Ummon.prototype.getTaskSync = function (tid) {
  ...
  // Clone the task
  const task = JSON.parse(JSON.stringify(self.tasks[tid]));
  const defaults = self.defaults[task.collection] || {};
  const globalDefaults = self.config.globalTaskDefaults || {};
...
Ummon.prototype.getTasks = function (filter, callback) {
  const self = this;
  const results = [];
  const allCollections = self.getCollections();
...
Ummon.prototype.createWorkerIfReady = function () {
  ...
  if (self.ready()) {
    const nextRun = self.queue.getNext(self.getRunningTaskIds());
    if (nextRun) {
...
Ummon.prototype.createWorker = function (run) {
  ...
  const workerProcess = worker(run, self);
