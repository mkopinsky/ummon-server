...
const ummon = { emit(name) {}, log: { error(err) {} } }; // emulate event emitter

const testQueue = queue({ maxSize: 4 }, ummon);
...
test('Test retreiving the next item from the queue that is not currently running', function (t) {
  ...
  const item = testQueue.getNext(['two']);
...
test('Make sure an empty queue doesn\'t return undefined', function (t) {
  ...
  const item2 = testQueue.getNext(['two']);
